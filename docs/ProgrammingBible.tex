...
\newcommand{\ilc}[1]{\texttt{#1}}%InLineCode, for names of classes and such in standard text.
\maketitle
...
(\path{C:\Program Files (x86)\National Instruments\Shared\Firmware\cRIO\76F2} on Windows 64-bit),
...
\begin{enumerate}
\item Connect to the roboRIO using USB, \textbf{not Ethernet}.
\item Launch the imaging tool, located at \path{C:\Program Files (x86)\National Instruments\LabVIEW 2015\project\roboRIO Tool\roboRIO_ImagingTool.exe} for 64-bit Windows.
...
\item Connect to the roboRIO using USB, \textbf{not Ethernet}.
\item Open the FRC Java Installer, located at \path{~/wpilib/tools/java-installer.jar}.
...
In order for the robot code to run, a class in the package \ilc{org.usfirst.frc.team\#\#\#\#.robot.Robot.java} must exist (Where \#\#\#\# is the team number) and extend \ilc{RobotBase} in some fashion.
To do this, WPILib provides \ilc{SampleRobot.java}, \ilc{CommandBasedRobot.java}, and \ilc{IterativeRobot.java}, and most robot code should extend one of these three classes.
...
\item Navigate to \path{~/wpilib/java/current/java/lib}.\footnote{\path{~} refers to the home directory, located at \path{/Users/username/} on Mac, \path{C:\Users\username} on Windows, and \path{/home/username} on UNIX.}
...
\item Therefore, edit them and set the "Classes" and "Sources" to their location on disk (\path{~/wpilib/java/current/lib}).
\end{enumerate}
...
Similarly to WPILib, a \ilc{Robot.class} must exist, and it still extends \ilc{RobotBase} indirectly.
However, instead of defining its own components, the \ilc{ObjectHolder} interface is used to define and hold all robot components.
The \ilc{Robot} class calls the \ilc{teleUpdateAll()} method on the \ilc{ObjectHolder} in order to \ilc{teleUpdate()} all of the components.

The \ilc{ObjectHolder} can either be entirely custom written, or can extend \ilc{SampleRobotObjectHolder}, which defines an \ilc{ArrayList} to store the components and iterates over each element in the list and calls update on them when necessary.
In the constructor of the \ilc{ObjectHolder}, it should declare all of the \ilc{RobotComponents} and pass in all data to properly initialise them.

The body of \ilc{Robot.class} should initialise an \ilc{ObjectHolder}, and in the \ilc{teleOperated()} method it should repeatedly call \ilc{teleUpdateAll()} on it.
...
It provides the \ilc{teleUpdate()} and \ilc{autoUpdate()} methods.
These should all be contained in the \ilc{ObjectHolder}.

\subsubsection{ObjectHolder}
This interface is for containers for \ilc{RobotComponent}s and similar.
It contains methods to call the \ilc{teleUpdate()} and \ilc{autoUpdate()} methods for its \ilc{RobotComponent}s.

\subsubsection{Movement}
\paragraph{TurtleMotor}
An interface representing a motor that is given a \ilc{MotorValue} to move, representing full forward to full backwards.
...
\ilc{TurtleDigitalInput} represents an input which has discrete values.
\paragraph{TurtleAnalogInput}
\ilc{TurtleAnalogInput} represents an input which has continuous values.
...
\ilc{SampleRobotObjectHolder} implements \ilc{ObjectHolder}, it is abstract and defines an \ilc{ArrayList} to store \ilc{RobotComponent}s.
It also defines the \ilc{teleUpdateAll()} and \ilc{autoUpdateAll()} methods which simply iterate over the \ilc{ArrayList} and update each component.
...
There is also a \ilc{FakeMotor} which emulates a motor controller but in fact does nothing.

\subsubsection{Sensors}
\paragraph{TurtleXtrinsicMagnetometer}
\ilc{TurtleXtrinsicMagnetometer} is a class for an Xtrinsic magnetometer.
It has 3 \ilc{TurtleRotationSensor}s representing the pitch, yaw, and roll axes.
\paragraph{TurtleAnalogGyro}
\ilc{TurtleAnalogGyro} is a wrapper for WPILib's \ilc{AnalogGyro} class.
\paragraph{TurtleDistanceEncoder}
\ilc{TurtleDistanceEncoder} is an encoder that measures distance travelled.
The scale between encoder ticks and inches must be passed in.
\paragraph{TurtleRotationEncoder}
\ilc{TurtleRotationEncoder} is an encoder that measures amount rotated.
The scale between encoder ticks and degrees must be passed in.

\subsubsection{Input}
\paragraph{TurtleJoystickAxis}
\ilc{TurtleJoystickAxis} implements \ilc{TurtleAnalogInput}, it takes a single axis from a Joystick and turns it into an \ilc{AnalogInput} with the range $[-1,1]$.
\paragraph{TurtleJoystickButton}
\ilc{TurtleJoystickButton} implements \ilc{TurtleDigitalInput}.
It returns 1 if the button is pressed, and 0 otherwise.
\paragraph{TurtleJoystickPOVSwitch}
\ilc{TurtleJoystickPOVSwitch} implements \ilc{TurtleDigitalInput}.
It represents a POV switch, also known as a hat switch or a d-pad with diagonals.
It returns -1 to 7, but it also contains an enum called \ilc{POVValue} which can accept this number and return a more usable value, such as North, South, and Southeast.
\paragraph{TurtleFlightStick}
This class represents the entire joystick for a flight stick, often used by FRC teams.
It contains several \ilc{TurtleJoystickAxis}, which can be retrieved from it.
It also contains a \ilc{TurtleJoystickPOVSwitch} and multiple \ilc{TurtleJoystickButton}.
...
\subsection{Utility}
This section covers utilities included in TurtleShell, found in the \ilc{util} package.
...
\paragraph{Time}
This implements \ilc{Unit}, and represents time in seconds.
\paragraph{MotorValue}
This implements \ilc{Unit}, and represents a motor power, from -1 to 1.
\paragraph{Distance}
This implements \ilc{Unit}, and represents a distance in inches.
\paragraph{Angle}
This implements \ilc{Unit}, and represents an angle in degrees.
\paragraph{Rate}
This implements \ilc{Unit}. It is generified, so it represents another \ilc{Unit} per \ilc{Time}.

\subsubsection{TurtleMaths}
\ilc{TurtleMaths} is full of mathematical functions which have proven useful. All are public static, see the JavaDocs for documentation.
...
\ilc{TurtlePID} returns a single \ilc{MotorValue}, while \ilc{TurtleDualPID} returns an array of \ilc{MotorValue}s (with two values).
...
\ilc{TurtlePDD2}, or Turtle Proportional-Derivative-Double Derivative, is the main single-motor implementation.\footnote{Fun Fact: The use of PDD2, rather than PID, was learned from the SAS in Kerbal Space Program.}
It uses the error, the derivative, and second derivative.
It attempts to make all of these zero.

\paragraph{TurtleZeroPID}
\ilc{TurtleZeroPID} is a dummy controller, it always returns zero.
It can be used to disable a PID controller.

\paragraph{TurtleStraightDrivePID}
\ilc{TurtleStraightDrivePID} uses two \ilc{TurtlePDD2}s, along with the differences between them, to drive the robot straight for a certain difference.
It only uses the encoder readings, it does not have gyro support.

\paragraph{TurtleTurnPID}
\ilc{TurtleTurnPID} is similar to \ilc{TurtleStraightDrivePID}, but it turns in place rather than moving straight.
