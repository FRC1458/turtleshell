...
\section{Sensors}
	...
	It doesnât cover their coding or integration into software.
...
\subsubsection{Magnetometer}
		...
		In the context of the FRC, they are used to measure the Earthâs magnetic field and thus act as a compass, providing a reference point for rotation.
		...
		This can prove a problem, as the robotâs motors generate magnetic fields which can interfere with proper function.
...
\subsection{Position}
	\subsubsection{Accelerometer}
		...
		Accelerometers can still be used in autonomous for positioning, but arenât very useful beyond that.
...
This step is only neccesary to run Java programs.
\begin{enumerate}
...
PuTTY is a Windows program available that can fufill this function.
...
For example, if several million floating point calculations need to be done in order to execute a single motor movement, the code will be far too ineffecient to be effective.
However, eking every last bit of effeciency out of code requires destroying any pretense of readibality or maintainability.
...
This additional layer of abstraction comes with a cost however, Java code is almost always less effecient than code written in C or C++.
...
It has limited applicibality outside of FRC and lacks the massive repository of libraries that other languages have.
