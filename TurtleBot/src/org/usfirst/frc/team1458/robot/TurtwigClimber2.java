	...
	public void teleUpdate() {
		...
		case RAISED:
			if(Input.getXboxButton(XboxButton.Y)) {
				this.state=ClimberState.YANKING;
				timer.start();
			}
			break;
		case RAISING:
		...
		case UNFOLDED:
			...
			hookEncoder.reset();
			pid = new TurtlePDD2(TurtwigConstants.hookRaiseConstants, TurtwigConstants.hookLiftEncoderTicks,
		...
		case RETRACTING:
			powerWinch.set(MotorValue.zero);
			hookWinch.set(pid.newValue(new double[] {hookEncoder.getTicks(), hookEncoder.getRate()}));
			break;
		case YANKING:
			powerWinch.set(MotorValue.fullBackward);
			break;
		case FOLDED:
			break;
		case RAISED:
