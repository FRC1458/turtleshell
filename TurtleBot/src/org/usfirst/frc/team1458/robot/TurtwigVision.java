...
public class TurtwigVision implements TurtleVision, TurtleTheta, TurtleDistance {
	...
	NIVision.imaqColorThreshold(binaryFrame, image, 255, NIVision.ColorMode.HSV, TARGET_HUE_RANGE, TARGET_SAT_RANGE, TARGET_VAL_RANGE);
	...
	if (numParticles > 0) {
	    ...
	    if (targetRecognised) {
		distance = (89 - TurtwigConstants.cameraHeight) * (Math.sin(Math.PI - (VisionMaths.yToTheta(particle.yCentre) - TurtwigConstants.cameraAngle)))
