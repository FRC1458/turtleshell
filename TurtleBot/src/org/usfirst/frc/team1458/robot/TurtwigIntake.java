...
public class TurtwigIntake implements TurtleRobotComponent {
	...
	private TurtleLimitSwitch intakeTopLimit = new TurtleEncoderLimit(rEncoder, TurtwigConstants.INTAKEENCODERMAX,
			true);
	...
	public void teleUpdate() {
		if (Input.getXboxAxis(XboxAxis.LY) != 0) {
			...
			} else if (!intakeTopLimit.isPressed()) {
				// Intake top limit has been released
				intakeTopLimitFlag = false;
			}
			if (intakeTopLimit.isPressed() && Input.getXboxAxis(XboxAxis.LY) < 0) {
				TurtleLogger.warning("Attempting to move intake while limit switch is preventing it");
			} else {
			...
			double currentPos = TurtleMaths.fitRange(TurtleMaths.avg(lEncoder.getTicks(), rEncoder.getTicks()), 0,
					TurtwigConstants.INTAKEENCODERMAX);
			pid = new TurtleStraightDrivePID(TurtwigConstants.intakePIDConstants, currentPos,
					TurtwigConstants.intakePIDkLR, 0.0);
		} else {
